...
export default class BookingApiClient extends HttpClient {
  ...
  async getService(zipCode: string): Promise<ServiceResponse> {
    ...
    try {
      const response: AxiosResponse<ServiceResponse> = await this.instance.get(
        url
      );
  ...
  async getSymptoms(serviceId: number): Promise<SymptomsResponse> {
    ...
    try {
      const response: AxiosResponse<SymptomsResponse> = await this.instance.get(
        url
      );
      const { status } = response;
      if (status === 200) {
  ...
  async getServiceTimeSlots(
    ...
    cityId: number,
    serviceId: number
  ): Promise<AppointmentTimeSlotResponse> {
    const url =
      "/service-time-slots?date=" +
      date +
      "&city=" +
      cityId +
      "&service=" +
      serviceId;
    try {
      const response: AxiosResponse<AppointmentTimeSlotResponse> =
  ...
  async createAppointment(
    ...
    try {
      const response: AxiosResponse<AppointmentResponse> =
        await this.instance.post(url, appointment);
      const { status } = response;
      if (status === 201) {
        const { data } = response;
        return data;
  ...
  async createAccount(account: Account): Promise<AccountResponse> {
    const url = "/accounts";
    try {
      const response: AxiosResponse<AccountResponse> = await this.instance.post(
        url,
        account
      );
      const { status } = response;
