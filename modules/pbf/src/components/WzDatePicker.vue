      ...
      >
        <template v-slot="{ inputValue, togglePopover, showPopover }">
          ...
          <div class="" v-else>
            <label>{{ label }}</label>
            <div
              class="flex items-center w-full border px-md py-sm rounded-lg cursor-text"
              :class="[
            ...
            >
              <wz-icon
                name="calendar"
                class="mr-4 text-center mx-auto w-6 h-6 fill-tetriary"
              />

              <input
                class="w-full bg-transparent p-0 m-0 border-none focus:ring-transparent focus:outline-none"
                :value="inputValue"
            ...
            <div class="text-xs text-red-600" v-show="errorMessage">
              {{ errorMessage }}
            </div>
          </div>
        </template>
